...
function AzurePriceComp(props){
    ...
    const virMac = azObj.value.filter(word => word.resourceType === 'virtualMachines');
    ...
    async function azureJSON(prod){
        const azureCall = `https://prices.azure.com/api/retail/prices?$filter=location eq 'US East 2' and skuName eq '` + prod + `' and serviceName eq 'Virtual Machines'`;
        ...
        if (!setInfoOnce){
            ...
            console.log(azureInfo);
            console.log("Fifty one and fifty two");
            if ((priceData.Items[0].retailPrice < azureInfo.retailPrice) || azureInfo.retailPrice === ''){
                // This is the hook that will set the price to the object which contains the lowest price
                console.log('Line 56', priceData.Items[0])
